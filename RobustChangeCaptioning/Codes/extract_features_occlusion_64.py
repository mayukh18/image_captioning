...
def main(args):
  ...
  for fn in os.listdir(args.input_image_dir):
    ...
    idx = int(os.path.splitext(fn)[0].split('.')[0].split('_')[-1]) ### TODO ###

    input_paths.append((os.path.join(args.input_image_dir, fn), idx))
    #print("eeeeeeeeeeeeeee " + str(input_paths))
    idx_set.add(idx)
  ...
  assert min(idx_set) == 0 and max(idx_set) == len(idx_set) - 1  ### TODO ###
  ...
  with h5py.File(args.output_h5_file, 'a') as f:
    feat_dset = None
    i0 = 0
    cur_batch = []
    ...
    for i, (path, idx) in enumerate(input_paths):
      ...
      if len(cur_batch) == args.batch_size:
        ...
        if feat_dset is None:
          N = len(input_paths)
          _, C, H, W = feats.shape
          feat_dset = f.create_dataset('images_features', (N, C, H, W),
                                       dtype=np.float32)
        i1 = i0 + len(cur_batch)
